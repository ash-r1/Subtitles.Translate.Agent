---
description: 出力生成と最終検査（Step 8–9）— タイミング調整・SRT 生成・全体整合性監査
...
$ARGUMENTS: `--skip-timing` で Step 8 を飛ばす、`--audit-only` で Step 9 のみ。

## 手順

1. **統合**: Step 7 までの確定訳（polished_text ?? 前工程の final_translation）
   と `work/source.json` の timecode を突き合わせ、
   `work/final.json`（`{id, start, end, text}` の配列）を作る。
   件数と ID が元字幕と完全一致することを機械的に確認する。

2. **Step 8 タイミング調整**（Haiku / low）:
   - まず `python3 scripts/check_constraints.py work/final.json
     subtitle-constraints.yaml` で違反一覧を機械抽出する。
   - 違反のある行（＋前後 1 行）だけをバッチにまとめ、
     `prompts/08-timing-adjust.md` でサブエージェントに調整させる
     （違反ゼロの行は LLM に渡さない）。
   - `needs_shortening` flag の行は Step 7 へ差し戻して縮約し、再検査する。
...
4. **Step 9 最終整合性監査**（Opus / high）:
   ...
   - 文脈検査（一人称・呼称・敬語・関係変化・orthography）は
     `prompts/09-final-audit.md` でサブエージェントに実行させる。
     検査項目別に並列分割してよい。
   - 出力 `work/09-final-audit.json`。
...
- [ ] Step 9 の findings がすべて severity・evidence 付きで報告された
- [ ] 未承認の自動一括置換を行っていない
