...
- **作品に通底する世界観・雰囲気の読み解き**（前提・世界の論理・空気感・
  緊張構造・笑いの質・感情の流れ）と、そこから導かれる翻訳への含意
- 作品種別と全体の語調
- 想定視聴者
- 時代・場所・作品の主題
...
個別の判断（語調・語彙水準・避ける表現・人物設計への申し送り）は、
すべて**世界観・雰囲気の読みを根拠**として導くこと。読みが先、規則が後。
...
2. **世界観・雰囲気を読み解く**（`world_and_atmosphere`）。表面的なジャンル
   ラベルで済ませず、次を言語化する:
   - **前提と世界の論理**: この世界は何が成り立っていて、何が普通と違うか
     （例: 登場人物はデジタル空間に囚われており、死や身体の概念が現実と異なる）
   - **空気感と緊張構造**: 作品の支配的な空気と、その中の対立軸
     （例: ポップで陽気な見た目と実存的な恐怖が同居し、落差自体が演出）。
     トーンが切り替わる場面の型（どういう瞬間に空気が変わるか）も記す
   - **笑いの質**: 笑いがあるなら、その種類（皮肉・メタ・ドタバタ・ブラック）
     と、誰が笑いを作り誰が受けるか
   - **感情の流れ**: 作品（またはエピソード）を通した感情の弧
   - **翻訳への含意**: 以上から導かれる日本語字幕上の指針
     （例: 明るい台詞ほど軽薄に振り切ってよい／恐怖の場面では語彙を
     現実的に引き締め、落差を殺さない／世界固有の概念語は説明的に訳さない）
3. `{{project_config}}` に公式情報（ジャンル・視聴者・既存の表記規則）が
   あれば最優先で採用し、`source: "project_config"` を付ける。
4. 語調・視聴者・時代設定を判定する。判定できない項目は `unknown` とし、
   `unresolved_questions` に理由を書く。
5. 日本語字幕の表記方針（orthography）を作品単位で決定する。
   `{{subtitle_constraints}}` に指定があればそれに従う。
6. 敬語の基本方針・語彙水準・禁止表現を、**手順 2 の読みを根拠に**決める
   （なぜその基準かを world_and_atmosphere から説明できること）。
7. 後続工程への注意事項（Step2 で重点的に見るべき人物・場面、トーン切替の
   起きる範囲など）をまとめる。
...
```json
{
  ...
  "world_and_atmosphere": {
    "premise": "作品の前提・世界の成り立ち（ネタバレ込み。後続工程用）",
    "world_rules": ["この世界固有のルール・現実との差分"],
    "atmosphere": "支配的な空気感と、その中の対立軸・緊張構造",
    "tonal_shifts": ["トーンが切り替わる場面の型（例: 日常ギャグ→実存的恐怖への急転）"],
    "humor_style": "笑いの質と担い手。笑いのない作品なら null",
    "emotional_arc": "作品を通した感情の弧",
    "translation_implications": ["この読みから導かれる日本語字幕上の指針（具体的に）"]
  },
...
- [ ] 翻訳文を含んでいない
- [ ] world_and_atmosphere が埋まっており、translation_implications が
      具体的（「自然に訳す」のような一般論だけになっていない）
- [ ] register_baseline・avoid_expressions が world_and_atmosphere の読みから
      説明できる
