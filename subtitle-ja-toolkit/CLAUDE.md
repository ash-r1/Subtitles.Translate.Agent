...
エージェント実行型のプロンプト・ツールキット。元になった C# アプリ
（リポジトリルートの `src/`）の 6 段階パイプラインを、日本語字幕向けに
9 工程へ拡張し、Claude Code / Cowork から直接実行できるファイル一式として
再構成したもの。**C# アプリ自体は変更せず、実行もしない。**

`.claude/` が使えない環境（Cowork 等）でも、この CLAUDE.md と README.md の
手順だけで全工程を実行できる。各工程の詳細指示は `prompts/01`〜`09` にあり、
実行時は該当ファイルを読み、`{{variable}}` を実データで置換して従う。
