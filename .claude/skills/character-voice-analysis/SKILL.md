...
- 冒頭の印象で全編の口調を固定しない。主人公・メインキャラクターは
  人物像の変化・成長（気弱→気丈 等）が典型的なので必ず検査し、あれば
  `character_arc` の phases（valid_range・voice_changes・evidence 付き）に
  記録する。変化がなければ null（無理に作らない）。
- 話者が確定できない字幕は `unknown` または候補一覧。断定しない。
- gender だけから一人称・語尾・性格を決めない。
- 知的水準・話術・得意分野（speech_register）を人物ごとに設計する。
  全員を作品平均の語彙水準・流暢さに均さない（口下手は口下手のまま）。
