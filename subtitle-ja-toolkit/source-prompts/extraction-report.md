...
> 註: 本レポート中の工程番号は抽出当時の 9 工程構成のもの。現行パイプラインは
> Step 4 に翻訳前検証ゲート（`prompts/04-pretranslation-check.md`）が挿入された
> 10 工程構成で、旧 Step 4〜9 は現 Step 5〜10 に対応する。


調査日: 2026-07-12
調査ブランチ: `main`（コミット `064743c 增加润色步骤` 時点）

> **注**: 本レポート中の `src/...` のファイルパスは調査時点のもの。
> C# 実装は抽出完了後にリポジトリから削除されたため、現在は
> git 履歴（`064743c` 以前）でのみ参照できる。
