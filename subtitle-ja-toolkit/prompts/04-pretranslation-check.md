...
- 対象: `{{pilot_batches}}`。`auto` の場合、**主要人物が揃い、感情・場面の
  ...
  主要人物に `character_arc` がある場合、可能なら**異なる phase から
  1 バッチずつ**選び、変化の前後両方の口調を確認できるようにする。
...
人物口調サンプル欄（D）は主要人物ごとに性格が出る台詞 3〜5 行をパイロット
から抜粋する。`character_arc` がある人物は **phase ごとにサンプルを分け**、
変化の設計（overview・phase 境界）自体も掲示して確認を取る（パイロット
範囲外の phase は設計の記述のみでよい）。
